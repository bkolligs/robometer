...
Configuration file: `robometer/dataset_upload/configs/data_gen_configs/new_mit_franka.yaml`
...
```bash
uv run python -m robometer.dataset_upload.generate_hf_dataset --config_path=robometer/dataset_upload/configs/data_gen_configs/new_mit_franka.yaml
```

To only process external camera views (exclude wrist camera):

```bash
uv run python -m robometer.dataset_upload.generate_hf_dataset --config_path=robometer/dataset_upload/configs/data_gen_configs/new_mit_franka.yaml --dataset.exclude_wrist_cam=true
```
