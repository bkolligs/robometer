...
def sample_prefix_frames(frames: np.ndarray, max_frames: int) -> np.ndarray:
    if max_frames < 1:
    ...
    frame_indices = np.rint(np.linspace(0, total_frames - 1, max_frames)).astype(int).tolist()
    frame_indices[0] = 0
...
def compute_expanded_rewards_per_frame(
    ...
    task: str,
    timeout_s: float = 120.0,
    expand_max_frames: int = DEFAULT_EXPAND_MAX_FRAMES,
) -> tuple[np.ndarray, np.ndarray]:
    """
    Expand a trajectory into prefixes on the client and assemble per-frame outputs.

    Each prefix is uniformly sampled down to ``expand_max_frames`` before inference
    so every request stays close to the frame budget used during training.
    """
    total_frames = int(video_frames.shape[0])
    rewards_list: list[float] = []
    success_probs_list: list[float] = []
    saw_success = False

    for prefix_length in range(1, total_frames + 1):
        prefix_frames = sample_prefix_frames(video_frames[:prefix_length], max_frames=expand_max_frames)
        prefix_rewards, prefix_success_probs = compute_rewards_per_frame(
            eval_server_url=eval_server_url,
            video_frames=prefix_frames,
            task=task,
            timeout_s=timeout_s,
            use_frame_steps=False,
        )
        rewards_list.append(float(prefix_rewards[-1]) if prefix_rewards.size > 0 else 0.0)
        if prefix_success_probs.size > 0:
            saw_success = True
            success_probs_list.append(float(prefix_success_probs[-1]))
        else:
            success_probs_list.append(0.0)

    rewards = np.array(rewards_list, dtype=np.float32)
    if not saw_success:
        return rewards, np.array([], dtype=np.float32)
    return rewards, np.array(success_probs_list, dtype=np.float32)
...
def main() -> None:
    ...
    parser.add_argument(
        "--expand",
        action="store_true",
        help="If set, expand client-side into prefixes (0:1, 0:2, ...) and assemble per-frame outputs afterwards",
    )
    parser.add_argument(
        "--expand-max-frames",
        type=int,
        default=DEFAULT_EXPAND_MAX_FRAMES,
        help=f"Maximum frames to send per expanded prefix (default: {DEFAULT_EXPAND_MAX_FRAMES})",
    )
    parser.add_argument(
        "--success-threshold",
        type=float,
    ...
    if args.expand:
        rewards, success_probs = compute_expanded_rewards_per_frame(
            ...
            expand_max_frames=int(args.expand_max_frames),
        )
    else:
